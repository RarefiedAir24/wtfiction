...
export function formatCitation(citation: Citation): React.ReactNode {
  ...
  if (citation.citation) {
    ...
    if (citation.title && citation.type) {
      ...
      const year = yearMatch ? yearMatch[1] : (citation.publishDate 
    ...
    if (citation.authors && citation.authors.length > 0 && citation.title) {
      ...
      const year = citation.publishDate 
        ? new Date(citation.publishDate).getFullYear().toString()
        : '';
      ...
      if (year && type) {
        return (
          <>
            {authors}, <em>{citation.title}</em> ({type}, {year}).
          </>
        );
      } else if (type) {
        return (
  ...
  if (citation.title) {
    ...
    const year = citation.publishDate 
      ? `, ${new Date(citation.publishDate).getFullYear()}`
      : '';
    const closing = type ? ')' : '';
    
    if (authors || type || year) {
      return (
        <>
          {authors}<em>{citation.title}</em>{type}{year}{closing}.
        </>
      );
    }
